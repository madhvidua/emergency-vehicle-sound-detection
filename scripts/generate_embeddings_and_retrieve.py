...
def main():
    ...
    parser.add_argument('--chunk_length', type=int, choices=[3, 5], required=True,
                        help="Length of the audio chunks in seconds (3 or 5)")
    parser.add_argument('--query_audio_path', type=str, required=True,
                        help="Path to the query audio file for retrieval")
    ...
    try:
        ...
        top_k = 100
        print(f"Top {top_k} similar audio files and their similarity scores:")

        for i in range(top_k):
            index = similarity_indices[i]
            similarity_score = similarities[index]
            path, _ = filtered_embeddings[index]
            print(f"idx: {index} Audio file: {path}, Similarity score: {similarity_score}")
